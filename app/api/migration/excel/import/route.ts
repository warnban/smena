...
import { importExcelWorkbook } from "@/lib/migration/excel-import.server";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await importExcelWorkbook(session.seatId, buffer);
    return NextResponse.json(result);
  } catch (e) {
    console.error("[migration/excel/import]", e);
